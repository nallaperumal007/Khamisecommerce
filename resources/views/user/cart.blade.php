...
function renderCart(){
    ...
    document.querySelectorAll('.increaseItem').forEach((btn,i)=>{
        btn.addEventListener('click',()=>{ window.cart[i].quantity++; window.cart[i].total = window.cart[i].quantity * window.cart[i].price; renderCart(); });
    });
    document.querySelectorAll('.decreaseItem').forEach((btn,i)=>{
        btn.addEventListener('click',()=>{ if(window.cart[i].quantity>1){ window.cart[i].quantity--; window.cart[i].total = window.cart[i].quantity * window.cart[i].price; renderCart(); } });
    });
    document.querySelectorAll('.removeItem').forEach((btn,i)=>{
        btn.addEventListener('click',()=>{ window.cart.splice(i,1); renderCart(); });
    });
});
...
document.querySelectorAll('#addToCartBtn').forEach(btn=>{
    btn.addEventListener('click',()=>{
        addToCart(btn.dataset.id, btn.dataset.name, parseFloat(btn.dataset.price), parseInt(document.getElementById('quantity').value));
        document.getElementById('quantity').value = 1;
    });
});
</script>
<style>
.btn-success{background-color:#4b752a;border:none;}
.btn-success:hover{background-color:#3d6023;}
</style>
